-- Throttle free usage by IP+UA: allow at most N distinct devices per IP+UA window
-- Configure N via constant below (can be adjusted in future migration)
...
declare
  ...
  v_month_end date;
  v_updated_id uuid;
  v_free_cap int := 2; -- max distinct devices per IP+UA per 30 days
...
begin
  ...
  if not v_has_device then
    ...
    -- Create device row with fingerprint
  ...
  -- If device exists and free not yet used, consume
